...
def extract(name, lowerRiskScore, higherRiskScore, fromDate, toDate, savePath):
    ...
    for patientRow in patientRows:
        responses = FhirUtils.readData('http://10.172.235.4:8080/fhir/Encounter?subject=' + patientRow['patient_id'] + '&_sort=date&_offset=0&_count=20')
        for response in responses:
            if 'entry' in response:
                entries = response['entry']
                for entry in entries:
                    dataRow = {
                        'patient_id': patientRow['patient_id'][1:],
                        'encounter_id': entry['resource']['id'][1:],
...
if __name__ == '__main__':
    ...
    parser.add_argument('-sp', '--save_path', nargs=1, default=['/tmp/demographics.csv'],
